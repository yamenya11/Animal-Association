...
class CourseController extends Controller
{
    ...
    public function __construct(CourseService $courseService)
    {
        ...
        public function recordView($courseId)
        {
            $userId = auth()->id();
            $result = $this->courseService->addView($courseId, $userId);
            
            return response()->json($result);
        }

       public function toggleLike($courseId)
    {
        try {
            $result = $this->courseService->toggleLike($courseId, auth()->id());
            
            return response()->json([
                'success' => true,
                'message' => $result['action'] === 'liked' ? 'تم الإعجاب بالكورس' : 'تم إلغاء الإعجاب',
                'is_liked' => $result['is_liked']
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'فشل في تحديث حالة الإعجاب: ' . $e->getMessage()
            ], 500);
        }
    }
     ...
     public function getLikes($courseId)
{
    ...
    {
        try {
            $stats = $this->courseService->getCourseStats($courseId, auth()->id());
    ...
    {
        try {
            $stats = $this->courseService->getDoctorStats(auth()->id());
